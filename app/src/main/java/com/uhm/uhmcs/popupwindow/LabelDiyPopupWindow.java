...
public class LabelDiyPopupWindow {
    ...
    private void fillGoodsData() {
        if (goodsModel != null) {
            ...
            // 3. 【核心修复】统一条码与下方文字逻辑
            // 逻辑：优先取 sn，如果没有 sn 则取 goods_sn。确保两处显示完全一致。
            String barcodeStr = TextUtils.isEmpty(goodsModel.getSn()) ? goodsModel.getGoods_sn() : goodsModel.getSn();

            if (!TextUtils.isEmpty(barcodeStr)) {
                // 强制让条码下方的 TextView 显示的内容与条码线条内容 100% 同步
                tvCode.setText(barcodeStr);
                ivBarcode.setImageBitmap(generateBarcode(barcodeStr));
            } else {
                tvCode.setText("");
            ...
            // 5. 积分与代金券显示 (优化：如果为 null 或空，默认显示 0.00)
